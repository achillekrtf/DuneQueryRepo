...
-- query name: Whale Flow Tracker
-- query link: https://dune.com/queries/6846428


-- ============================================================
-- Whale Flow Tracker — INCREMENTAL VERSION (fixed)
-- Daily net flow: 🐳 Whale (Top 10) vs 🐠 Rest — All chains
-- ============================================================
...
--    scan_cutoff   : 1 day back → recompute window
--    price_start   : 4 days back → fill-forward safety
-- ==========================================================
checkpoint AS (
    SELECT
        COALESCE(MAX(day), TIMESTAMP '2024-04-30')                                        AS output_cutoff,
        CAST(COALESCE(MAX(day), TIMESTAMP '2024-04-30') - INTERVAL '1' DAY AS DATE)       AS scan_cutoff_date,
        CAST(COALESCE(MAX(day), TIMESTAMP '2024-04-30') - INTERVAL '1' DAY AS TIMESTAMP)  AS scan_cutoff_ts,
        CAST(COALESCE(MAX(day), TIMESTAMP '2024-04-30') - INTERVAL '4' DAY AS DATE)       AS price_start_date,
        CAST(COALESCE(MAX(day), TIMESTAMP '2024-04-30') - INTERVAL '4' DAY AS TIMESTAMP)  AS price_start_ts
    FROM prev
),

-- ==========================================================
-- 2. EVM FULL HISTORY — single scan, serves both:
--    a) current_balances (whale classification)
--    b) new daily flows (from scan_cutoff)
--    FIX: one UNNEST pass instead of 4 separate scans
-- ==========================================================
evm_all AS (
    SELECT
        ...
        product,
        wallet,
        CASE WHEN side = 'in' THEN amount ELSE -amount END AS amount
    FROM (
        SELECT
            evt_block_time,
            CASE
...
-- ==========================================================
-- 3. STARKNET FULL HISTORY — single scan
--    FIX: UNNEST for in/out, cardinality guard on keys[3]
-- ==========================================================
starknet_all AS (
    SELECT
...
-- ==========================================================
-- 4. STELLAR DELTAS — with sentinel-row baseline
...
stellar_deltas AS (
    SELECT day,
        CASE contract_id
            WHEN 'CARUUX2FZNPH6DGJOEUFSIUQWYHNL5AVDV7PMVSHWL7OBYIBFC76F4TO' THEN 'USTBL'
            WHEN 'CBGV2QFQBBGEQRUKUMCPO3SZOHDDYO6SCP5CH6TW7EALKVHCXTMWDDOF' THEN 'EUTBL'
            WHEN 'CDT3KU6TQZNOHKNOHNAFFDQZDURVC3MSTL4ML7TUTZGNOPBZCLABP4FR' THEN 'UKTBL'
            WHEN 'CDS2GCAQTNQINSCJUJIVBJXILKBWP5PU7LOBGHMP3X47QCQBFKPMTCNT' THEN 'SPKCC'
            WHEN 'CDWOB6T7SVSMMQN5V3P2OPTBAXOP7DAZHGVW3PYTZIKHVFKN6TBSXR6A' THEN 'eurSPKCC'
            WHEN 'CBOOCGZSVRSZFRE4U2NWR2B4RXYVJWRCBTGOUD2JPI2TDJPWMTJX7FZP' THEN 'SAFO-EUR'
            WHEN 'CDGSC6BA4TCAOVSFQCUEHDMOIIHYYVNYBT6YEARS4MX3ITAHUINVGQHX' THEN 'SAFO-USD'
            WHEN 'CAGYRRKPFSWKM6SJOE4QAAVYMOSHMDS5WOQ4T5A2E6XNCU7LZZKUNQKP' THEN 'SAFO-GBP'
            WHEN 'CAJD2IBSP7VO2VYJQUYJSOGPJINTUYV7MQITIINXVPTIH3CCLCUENNMW4' THEN 'SAFO-CHF'
        END AS product,
        holder AS wallet,
        balance - COALESCE(
    ...
    WHERE day >= (SELECT scan_cutoff_date FROM checkpoint)
),

-- ==========================================================
-- 5. CURRENT BALANCES — from full EVM + Starknet history
--    Used for whale classification (top 10 per product)
--    FIX: derived from evm_all + starknet_all (no extra scan)
-- ==========================================================
current_balances AS (
    SELECT product, wallet, SUM(amount) AS balance
    FROM (
        SELECT product, wallet, amount FROM evm_all
        UNION ALL
        SELECT product, wallet, amount FROM starknet_all
    ) t
    GROUP BY 1, 2
    HAVING SUM(amount) > 0
),
...
-- ==========================================================
-- 6. WHALE CLASSIFICATION — top 10 per product
-- ==========================================================
whale_set AS (
    SELECT product, wallet
    FROM (
        SELECT product, wallet,
            ROW_NUMBER() OVER (PARTITION BY product ORDER BY balance DESC) AS rk
...
-- ==========================================================
-- 7. NEW TRANSFERS ONLY — from scan_cutoff
--    FIX: filter evm_all and starknet_all to scan window
--    instead of re-scanning the table
-- ==========================================================
new_transfers AS (
    SELECT day, product, wallet, SUM(amount) AS net_flow
    FROM (
        SELECT day, product, wallet, amount
        FROM evm_all
        WHERE day >= (SELECT CAST(scan_cutoff_ts AS DATE) FROM checkpoint)
        UNION ALL
        SELECT day, product, wallet, amount
        FROM starknet_all
        WHERE day >= (SELECT scan_cutoff_date FROM checkpoint)
...
-- ==========================================================
-- 8. NAV PRICES — fill-forward
--    FIX: replaced fusion/ranks/eth_nav_daily with standard
--    eth_ustbl/eutbl_prices + WINDOW pattern
--    FIX: added WINDOW fill-forward for USTBL/EUTBL
--    FIX: added SAFO oracle prices from Arbitrum
-- ==========================================================
days AS (
...
arb_prices AS (
    SELECT day,
        ...
        -- SAFO NAV prices (6 decimal precision)
...
filled_prices AS (
    SELECT d.day,
        ...
        -- SAFO NAV prices (6 decimal precision from oracle)
        ...
        COALESCE(last_value(fx.eur_raw) IGNORE NULLS OVER w * 1e-8, 1.08) AS rate_eur_usd,
        COALESCE(last_value(fx.gbp_raw) IGNORE NULLS OVER w * 1e-8, 1.27) AS rate_gbp_usd
    FROM days d
...
tagged_new AS (
    SELECT
        ...
        f.product,
        f.wallet,
        CASE WHEN w.wallet IS NOT NULL THEN '🐳 Whale (Top 10)' ELSE '🐠 Rest' END AS segment,
        f.net_flow,
        f.net_flow
            * CASE f.product
            ...
            * CASE f.product
                WHEN 'USTBL'    THEN 1.0
                WHEN 'EUTBL'    THEN pr.rate_eur_usd
                WHEN 'UKTBL'    THEN pr.rate_gbp_usd
                WHEN 'SPKCC'    THEN 1.0
                WHEN 'eurSPKCC' THEN pr.rate_eur_usd
                WHEN 'SAFO-EUR' THEN pr.rate_eur_usd
                WHEN 'SAFO-USD' THEN 1.0
                WHEN 'SAFO-GBP' THEN pr.rate_gbp_usd
                WHEN 'SAFO-CHF' THEN pr.rate_eur_usd / pr.rate_eur_usd * 1.13
              END AS net_flow_usd
    FROM new_transfers f
    LEFT JOIN whale_set     w  ON f.product = w.product AND f.wallet = w.wallet
    LEFT JOIN filled_prices pr ON f.day = pr.day
    WHERE f.net_flow != 0
),

-- ==========================================================
-- 10. NEW ROWS — aggregated
-- ==========================================================
new_rows AS (
    SELECT
        day,
        product,
        segment,
        ROUND(SUM(net_flow), 2)     AS net_flow,
        ROUND(SUM(net_flow_usd), 0) AS net_flow_usd,
...
-- ==========================================================
-- 11. OUTPUT — gap-safe boundary
-- ==========================================================
SELECT day, product, segment, net_flow, net_flow_usd, active_wallets
FROM prev
...
ORDER BY day DESC, product, segment