...
-- query name: TVL Growth Current Month
-- query link: https://dune.com/queries/6737416


-- ============================================================
-- KPI #2 — 30d Rolling TVL Growth — CURRENT SNAPSHOT
-- Method: supply_today vs supply_today − Σ(last 30d changes)
-- ============================================================

WITH

-- ==========================================================
-- 1. EVM SUPPLY TODAY — from genesis
-- ==========================================================
evm_supply_today AS (
...
-- ==========================================================
-- 2. EVM DELTA LAST 30 DAYS — net mints/burns in window
--    FIX: single scan, reuses same CASE mapping
-- ==========================================================
evm_last30 AS (
    SELECT product, SUM(change) AS delta_30d
    FROM (
        SELECT
            CASE
                WHEN contract_address = 0xe4880249745eAc5F1eD9d8F7DF844792D560e750 AND blockchain IN ('ethereum','polygon','base','etherlink') THEN 'USTBL'
            ...
            END / 1e5 AS change
        FROM evms.erc20_transfers
        WHERE evt_block_time >= CAST(current_date - INTERVAL '30' DAY AS TIMESTAMP)
          AND blockchain IN ('ethereum','polygon','arbitrum','base','etherlink')
          AND contract_address IN (
              -- MMF + SPKCC
              0xe4880249745eAc5F1eD9d8F7DF844792D560e750, 0x021289588cd81dC1AC87ea91e91607eEF68303F5,
...
-- ==========================================================
-- 3. STARKNET SUPPLY TODAY
-- ==========================================================
starknet_supply_today AS (
    SELECT product, SUM(change) AS net_supply
    FROM (
        SELECT
            CASE
...
-- ==========================================================
-- 5. STELLAR SUPPLY TODAY + DELTA LAST 30 DAYS
--    Single full-history scan, filter after LAG
-- ==========================================================
stellar_raw AS (
    SELECT closed_at,
...
-- ==========================================================
-- 6. COMBINED SUPPLY TODAY + 30D DELTA
-- ==========================================================
products AS (
...
supply_today AS (
    SELECT p.product,
        COALESCE(e.net_supply, 0)
        + COALESCE(s.net_supply, 0)
        + COALESCE(st.net_supply, 0) AS net_supply
    FROM products p
    LEFT JOIN evm_supply_today      e  ON p.product = e.product
    LEFT JOIN starknet_supply_today s  ON p.product = s.product
    LEFT JOIN stellar_supply_today  st ON p.product = st.product
),
delta_30d AS (
    SELECT p.product,
        COALESCE(e.delta_30d, 0)
        + COALESCE(s.delta_30d, 0)
        + COALESCE(st.delta_30d, 0) AS delta_30d
    FROM products p
    LEFT JOIN evm_last30      e  ON p.product = e.product
    LEFT JOIN starknet_last30 s  ON p.product = s.product
    LEFT JOIN stellar_last30  st ON p.product = st.product
),

-- ==========================================================
-- 7. LATEST NAV PRICES (last 5 days)
-- ==========================================================
recent_price_logs AS (
    ...
    SELECT date_trunc('day', block_time),
           0x29503f31B73F0734455942Eb888E13acA1588a4e, 'eth_eutbl_old',
    ...
    UNION ALL
    SELECT date_trunc('day', block_time),
           0xdaA1c6511Aa051e9e83Dd7Ac2D65d5E41D1f6b98, 'eth_eutbl_cl',
           bytearray_to_uint256(bytearray_substring(data, 1, 32))
    FROM ethereum.logs
    WHERE block_date >= current_date - INTERVAL '5' DAY
      AND topic0 = 0xc797025feeeaf2cd924c99e9205acb8ec04d5cad21c41ce637a38fb6dee6016a
...
latest_navs AS (
    SELECT src, contract_address, MAX_BY(price_raw, day) AS price_raw
    FROM recent_price_logs GROUP BY 1, 2
),
today_nav AS (
    SELECT
        COALESCE(
...
-- ==========================================================
-- 9. AuM TODAY + AuM 30D AGO per product
-- ==========================================================
aum_calc AS (
    SELECT
        t.product,
        ...
        END AS fx_rate,
        GREATEST(t.net_supply, 0)
            * CASE t.product WHEN 'USTBL' THEN n.p_ustbl WHEN 'EUTBL' THEN n.p_eutbl WHEN 'UKTBL' THEN n.p_uktbl WHEN 'SPKCC' THEN n.p_spkcc WHEN 'eurSPKCC' THEN n.p_eurspkcc WHEN 'SAFO-EUR' THEN n.p_safo_eur WHEN 'SAFO-USD' THEN n.p_safo_usd WHEN 'SAFO-GBP' THEN n.p_safo_gbp WHEN 'SAFO-CHF' THEN n.p_safo_chf END
            * CASE t.product WHEN 'USTBL' THEN 1.0 WHEN 'SPKCC' THEN 1.0 WHEN 'SAFO-USD' THEN 1.0 WHEN 'EUTBL' THEN fx.rate_eur_usd WHEN 'eurSPKCC' THEN fx.rate_eur_usd WHEN 'SAFO-EUR' THEN fx.rate_eur_usd WHEN 'UKTBL' THEN fx.rate_gbp_usd WHEN 'SAFO-GBP' THEN fx.rate_gbp_usd WHEN 'SAFO-CHF' THEN fx.rate_chf_usd END
            AS aum_today_usd,
        GREATEST(t.net_supply - COALESCE(d.delta_30d, 0), 0)
            * CASE t.product WHEN 'USTBL' THEN n.p_ustbl WHEN 'EUTBL' THEN n.p_eutbl WHEN 'UKTBL' THEN n.p_uktbl WHEN 'SPKCC' THEN n.p_spkcc WHEN 'eurSPKCC' THEN n.p_eurspkcc WHEN 'SAFO-EUR' THEN n.p_safo_eur WHEN 'SAFO-USD' THEN n.p_safo_usd WHEN 'SAFO-GBP' THEN n.p_safo_gbp WHEN 'SAFO-CHF' THEN n.p_safo_chf END
            * CASE t.product WHEN 'USTBL' THEN 1.0 WHEN 'SPKCC' THEN 1.0 WHEN 'SAFO-USD' THEN 1.0 WHEN 'EUTBL' THEN fx.rate_eur_usd WHEN 'eurSPKCC' THEN fx.rate_eur_usd WHEN 'SAFO-EUR' THEN fx.rate_eur_usd WHEN 'UKTBL' THEN fx.rate_gbp_usd WHEN 'SAFO-GBP' THEN fx.rate_gbp_usd WHEN 'SAFO-CHF' THEN fx.rate_chf_usd END
            AS aum_30d_ago_usd
    FROM supply_today t
    LEFT JOIN delta_30d d ON t.product = d.product
    CROSS JOIN today_nav n
    CROSS JOIN today_fx  fx
),
totals AS (
    SELECT
        SUM(aum_today_usd)  AS tvl_today,
        SUM(aum_30d_ago_usd) AS tvl_30d_ago
    FROM aum_calc
)
...
SELECT
    ac.product,
    ac.net_supply,
    current_date                                                           AS checkpoint_day,
    ROUND(ac.aum_today_usd, 0)                                            AS tvl_today,
    ROUND(ac.aum_30d_ago_usd, 0)                                          AS tvl_30d_ago,
    ROUND((ac.aum_today_usd - ac.aum_30d_ago_usd) / NULLIF(ac.aum_30d_ago_usd, 0) * 100.0, 2) AS growth_30d_pct,
    ROUND(tot.tvl_today, 0)                                                AS total_tvl_today,
    ROUND(tot.tvl_30d_ago, 0)                                              AS total_tvl_30d_ago,
    ROUND((tot.tvl_today - tot.tvl_30d_ago) / NULLIF(tot.tvl_30d_ago, 0) * 100.0, 2) AS total_growth_30d_pct
FROM aum_calc ac
